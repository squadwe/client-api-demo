$(function () {
    ...
    squadwe.inboxIdentifier = "bXKr2F2asDjztoVipLF9Lt8v";
    squadwe.squadweAPIUrl = "http://localhost:3000/public/api/v1/"
    ...
    connection.onopen = function () {
        ...
        // first we want users to subscribe to the squadwe webhooks
        connection.send(JSON.stringify({command:"subscribe", identifier: "{\"channel\":\"RoomChannel\",\"pubsub_token\":\""+squadwe.contactPubsubToken+"\"}" }));
    ...
    function setUpContact() {
      if(getCookie('contactIdentifier')){
        squadwe.contactIdentifier = getCookie('contactIdentifier');
        squadwe.contactPubsubToken = getCookie('contactPubsubToken')
      }else{
        xhttp.open("POST", squadwe.squadweAPIUrl + "inboxes/"+squadwe.inboxIdentifier+"/contacts");
        xhttp.send();
    ...
    function setUpConversation() {
      if(getCookie('contactConverstion')){
        squadwe.contactConverstion = getCookie('contactConverstion');
      }else{
        xhttp.open("POST", squadwe.squadweAPIUrl + "inboxes/"+squadwe.inboxIdentifier+"/contacts/"+squadwe.contactIdentifier+"/conversations", false);
        xhttp.send();
    ...
    function sendMessage(msg){
      xhttp.open("POST", squadwe.squadweAPIUrl + "inboxes/"+squadwe.inboxIdentifier+"/contacts/"+squadwe.contactIdentifier+"/conversations/"+squadwe.contactConverstion+"/messages", false);
